...
int noVer, algorithm, destination, size=1;                                  //declaration of global variables
...
void traverse(int source,int parent[])
{
	...
	while(R2==1)
	{
		if(counter<noVer-1)
		{
			...
			if( temp[counter]!= 0)
			{
				printf("You are currently at node %d and next node in the shortest path is %d. You want to keep traversing(1)or bring some Dynamic change?(0))",temp[counter],temp[counter+1]);
				scanf("%d",&R2);
				if(R2==1)
				counter++;
			}
...
// INITIALISING SINGLE SOURCE BY ADDING INFINITE IN ALL THE OTHER VERTICES IN WEIGHT ARRAY
	...
	height(2);
	distance(30);
	printf("\n Do you want to start traversing your shortest path? (1/0) \n");
	distance(30);
	printf("-->");
	scanf("%d",&R1);
