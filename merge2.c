...
// Astar function
void AstarMain(int source)
{   
    int src,des;
    //gettinng destination
    ...
    final->distance=0;
    final->via=source;
    final->Hfunc=final->distance+graph[source-1].heuristic;

    //initialising PQ
    PQ=(struct nodeForAStar *)malloc(1*sizeof(struct nodeForAStar));

    // calling function to create PQ using heap
    createArrayForPQ(graph[source-1],graph[source-1].noOfNodeConnected);
    ...
    {
        ABC[temp2->vertex-1]=temp2->prev->vertex;
        temp2=temp2->prev;
    }
